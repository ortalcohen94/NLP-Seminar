...
class NeuralNetModel (nn.Module):
    def __init__(self, hidden_size, embeddings_size, num_layers, text, tag, num_classes = 3, verbose = True):
        super().__init__()
        self.text = text
        self.tag = tag
        ...
        self.loss_function = nn.NLLLoss(reduction='sum', ignore_index=pad_id)

        self.premise_layer = nn.Sequential(
            nn.Embedding(self.V, embeddings_size, padding_idx = pad_id),
    ...
    def forward(self, premise_batch, hypothesis_batch):
        ...
        _, premise_layer_output = self.premise_layer(premise_batch)
        print(premise_layer_output.shape)
        _, hypothesis_layer_output = self.hypothesis_layer(hypothesis_batch),
        concat_output = torch.cat((premise_layer_output, hypothesis_layer_output))
        logits = self.log_soft_max(self.model_layers(concat_output))
        return logits
    ...
    def compute_loss(self, logits, ground_truth):
        return self.loss_function(logits.view(-1, self.N), ground_truth.view(-1))
