...
class StatisticsLocalModel(LocalModel):

    def __init__(self, y_p_explain, sentence_len, tokenizer):
        self.values = {key : list() for key in range(sentence_len)}
        self.y_p_explain = y_p_explain
        self.tokenizer = tokenizer

    def train(self, batch):
        for data, label in batch:
            self.values[data].append(label)
    ...
    def plot_explaination(explanations):
        ...
        StatisticsLocalModel.label_bar(rects2, ax, labels=names)
